# Main.gd v1.0 — Scene orchestrator
extends Node
...
func _show_title()->void:
	_title = load("res://scripts/ui/TitleScreen.gd").new()
	_title.name = "Title"
...
func _show_world_map()->void:
	_free_scene()
	var wm = load("res://scripts/ui/WorldMap.gd").new()
	wm.name = "WorldMap"
	wm.connect("enter_zone",   _on_enter_zone)
	wm.connect("show_dialog",  _on_wm_dialog)
	add_child(wm); move_child(wm, 0)
	_scene = wm

func _on_wm_dialog(lines:Array)->void:
	_dialog.show_lines(lines, Callable())
...
func _show_world(world_id:String)->void:
	...
	shape.radius = 10.0
	cshape.shape  = shape
	_player.add_child(cshape)
	world.add_child(_player)

	# Initialize world with player + UI references
...
func _on_battle_ended(won:bool, badge_name:String, xp:int)->void:
	...
	if won:
		...
		lines.append("Study with teachers and\nreturn stronger!")
		_dialog.show_lines(lines, Callable())
...
func _start_duel(data:Dictionary)->void:
	...
	d.setup(data.get("world","math"), data.get("opponent",{}), _dialog)
...
func _on_duel_ended(won:bool, xp:int)->void:
	...
	if _scene: _scene.show()
	if won:
		GameManager.add_xp(xp); GameManager.add_duel_win()
		_hud.show_xp_gain(xp)
		_dialog.show_lines(["Duel Victory!",GameManager.player_name+" wins the Knowledge Duel!",
			"+"+str(xp)+" XP!\nDuel Wins: "+str(GameManager.get_duel_wins())], Callable())
	else:
		GameManager.add_duel_loss()
		_dialog.show_lines(["Duel lost...","Study your weak topics and\nchallenge again!", "Every loss makes you stronger."], Callable())

func _free_scene()->void:
	if _scene and is_instance_valid(_scene): _scene.queue_free(); _scene = null
	if _player and is_instance_valid(_player): _player = null
