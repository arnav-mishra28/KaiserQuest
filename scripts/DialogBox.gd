...
var _blink_t:float=0.0; var _blink:bool=true
var _callback:Callable=Callable(); var _open:bool=false; var _ctrl:Control
...
func show_lines(lines:Array, cb:Callable=Callable())->void:
	...
	_begin(); _ctrl.show(); _open=true
	set_process(true); set_process_input(true); _notify(true)

func hide_dialog()->void:
	_ctrl.hide(); _open=false
	set_process(false); set_process_input(false); _notify(false)

func is_open()->bool: return _open

func _notify(v:bool)->void:
	var ow:=get_tree().get_first_node_in_group("overworld")
	if ow and ow.has_method("set_dialog_open"): ow.set_dialog_open(v)
...
func _finish()->void:
	hide_dialog(); if _callback.is_valid(): _callback.call()
...
class _Drawer extends Control:
	...
	func _draw()->void:
		...
		const W:=480; const H:=320; const BH:=84; const FS:=14
		var by:=H-BH-4; var fnt:=ThemeDB.fallback_font
		# Panel
		draw_rect(Rect2(4,by,W-8,BH),Color(0.03,0.03,0.09,0.94))
		draw_rect(Rect2(4,by,W-8,BH),Color(0.72,0.78,1.0,0.82),false,2.0)
		draw_rect(Rect2(6,by+2,W-12,BH-4),Color(0.35,0.4,0.72,0.30),false,1.0)
		# Name tag if present (first line starting with "NAME:")
		# Text
		var rows=db._shown.split("\n")
		for i in rows.size():
			draw_string(fnt,Vector2(14,by+9+FS+i*(FS+4)),
				rows[i],HORIZONTAL_ALIGNMENT_LEFT,W-28,FS,Color("#e8e8e8"))
		# Arrow
		...
		# Page counter
		if db._lines.size()>1:
			draw_string(fnt,Vector2(W-52,by+15),
