...
public class WorldController : MonoBehaviour
{
    ...
    const int TS = 32;
    float _holdT=0f, _stepT=0f;
    Vector2Int _lastDir=Vector2Int.zero;
    const float FIRST_DELAY=0.20f, HOLD_RATE=0.09f;
    ...
    void OnEnable()
    {
        ...
        if (DialogBox.Instance != null) DialogBox.Instance.SetDialogOpen(false);
    }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.F5)) {
            ...
            GameScreenManager.Instance?.GoTo(GameScreen.Title);
            return;
        }

        if (DialogOpen || _moving) { _holdT=0f; _stepT=0f; _lastDir=Vector2Int.zero; return; }
    ...
    void TryStep(Vector2Int dir)
    {
        ...
        if (WorldManager.Instance.TryMove(_gridPos, dir, out var dest)) {
            ...
            // Animate
            var target=new Vector2(dest.x*TS,dest.y*TS);
            StartCoroutine(SmoothMove(target));
            _animT+=0.10f;
            if(_animT>=0.10f){_animT=0f;_frame=1-_frame;}
        }
    }
    ...
    System.Collections.IEnumerator SmoothMove(Vector2 target)
    {
        ...
        while(t<dur){ t+=Time.deltaTime; _visualPos=Vector2.Lerp(start,target,t/dur); yield return null; }
        _visualPos=target; _moving=false;
        ...
        if(GameScreenManager.Instance?.Current==GameScreen.Battle){
            // Battle was triggered by gym interaction
            StartBattle();
        }
        if(GameScreenManager.Instance?.Current==GameScreen.Duel){
            StartDuel();
        }
    }
    ...
    void StartDuel()
    {
        DuelManager.Instance?.Setup(GameManager.Instance.BranchKey,
            new GymLeaderData{name="Rival Kira",color=new Color(0.88f,0.31f,0.06f)},0.55f);
    }
